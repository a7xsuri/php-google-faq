    ...
    $faq = [
        ...
        [
            ...
            'list' => [
                ...
                'sublist' =>[
                    '1' => "Google Ireland Limited, se gli utenti sono residenti nello Spazio economico europeo (paesi dell'Unione europea, oltre a Islanda, Liechtenstein e Norvegia) o in Svizzera.",
                    '2' => "Google LLC, con sede negli Stati Uniti, per il resto del mondo.",
                ],
...
<body>
    ...
    foreach($faq as $element){
        echo "<h3>{$element['question']}</h3>";
        echo "<p>{$element['answer']}</p>";
        if($element['list']){
            for($i = 1; $i < count($element['list']); $i++){
                if($element['list'][$i]['sublist']){
                     echo "<ul><li>{$element['list'][$i]}<ul><li>{$element['list']['sublist']['1']}</li><li>{$element['list']['sublist']['2']}</li></ul></li></ul>";
                }
            }
        }
    }
